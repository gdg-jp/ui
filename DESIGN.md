...
| radius | 小12px、入力・ボタン16px、カード20px、Dialog24px。円・pill・角丸なしは維持 |
...
カード、ボタン、aside、入力、ポップアップなどの輪郭は枠線ではなくドロップシャドウで表現します。操作部品と小さな面はsm、カードとポップアップはmd、Dialogはlgを使います。Sidebar、Header、Sheetは隣接する領域側だけへ影を落とします。ghostボタンには影を付けません。表、タブ、Accordion、Separatorのように情報を区切る線は、薄いborderトークンを使って残します。エラー境界とfocusは専用色で識別し、強制カラーモードでは影の代わりに枠線を表示します。通常文字4.5:1、必要な操作部品とfocusは3:1を下限とします。disabled以外の状態を不透明度低下だけで表しません。
