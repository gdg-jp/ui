...
余白、整列、文字の大小と太さで階層を作ります。通常画面は中立色を主体に、GDG Blueを主要操作へ集中させます。意味のない装飾、グラデーション、ガラス表現、重い影を使いません。ロゴやイベント固有の装飾は基本UIに混ぜず、公式アセットを無加工で使う場面に限定します。

| トークン | 値／割当 |
| --- | --- |
| white / white-2 | #FFFFFF / #F0F0F0 |
...
| border | Light=black-2の10%、Dark=white-2の12% |
| primary / on-primary | 両テーマとも#4285F4 / #141414 |
| red / green / yellow | #EA4335 / #34A853 / #F9AB00。アクセント用 |
| radius | 小6px、入力・ボタン8px、カード10px、Dialog12px |
...
ブランド色は固定値、操作上の意味はテーマ別の変数で表現します。通常サイズの白文字をGDG Blueへ直接置くと必要なコントラストを満たさないため、primaryの文字は濃色です。リンクの青は別トークンです。Red／Green／Yellowをそのまま小さな文字に使わず、danger／success／warningを使います。

装飾枠線は薄くしますが、InputやCheckboxの境界とfocusは専用色で識別します。通常文字4.5:1、必要な操作部品とfocusは3:1を下限とします。disabled以外の状態を不透明度低下だけで表しません。
