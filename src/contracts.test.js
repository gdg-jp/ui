...
describe("public contracts", () => {
    it("renders native button, loading state and disabled links on the server", () => {
        expect(renderToStaticMarkup(_jsx(UI.Button, { loading: true, children: "\u4FDD\u5B58" }))).toMatch(/disabled=""/);
        expect(renderToStaticMarkup(_jsx(UI.Button, { loading: true, children: "\u4FDD\u5B58" }))).toContain('aria-busy="true"');
        expect(renderToStaticMarkup(_jsx(UI.Button, { asChild: true, disabled: true, children: _jsx("a", { href: "/test", children: "\u79FB\u52D5" }) }))).toContain('aria-disabled="true"');
        expect(renderToStaticMarkup(_jsx(UI.Button, { children: "\u4FDD\u5B58" }))).toContain('type="button"');
    });
    it("associates field labels, descriptions and errors with SSR-stable ids", () => {
        const html = renderToStaticMarkup(_jsx(UI.FormField, { id: "email", label: "\u30E1\u30FC\u30EB", description: "\u9023\u7D61\u5148", error: "\u5165\u529B\u3057\u3066\u304F\u3060\u3055\u3044", required: true, children: _jsx(UI.Input, {}) }));
    ...
    it("has no blanket transitions or application dependencies", () => {
        expect(readFileSync(new URL("./styles/components.css", import.meta.url), "utf8")).not.toMatch(/transition:\s*all/);
