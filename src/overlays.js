...
import { AlertDialog as RA, Dialog as RD, DropdownMenu as RM, Popover as RP, Tooltip as RT, } from "radix-ui";
...
export function DialogContent({ ref, className, children, closeLabel = "閉じる", ...props }) {
    ...
    return (_jsxs(RD.Portal, { children: [_jsx(RD.Overlay, { className: "gdg-overlay" }), _jsxs(RD.Content, { ref: motionRef, ...props, className: cn("gdg-dialog", className), children: [children, _jsx(RD.Close, { asChild: true, children: _jsx(IconButton, { "aria-label": closeLabel, variant: "ghost", className: "gdg-dialog-close", children: _jsx(X, { size: 18 }) }) })] })] }));
}
...
export function SheetContent({ className, side = "left", ...props }) {
    return _jsx(DialogContent, { ...props, className: cn("gdg-sheet", `gdg-sheet-${side}`, className) });
}
...
export function AlertDialogContent({ ref, className, ...props }) {
    const motionRef = useMotionRef(ref);
    return (_jsxs(RA.Portal, { children: [_jsx(RA.Overlay, { className: "gdg-overlay" }), _jsx(RA.Content, { ref: motionRef, ...props, className: cn("gdg-dialog", className) })] }));
}
...
export function PopoverContent({ ref, className, ...props }) {
    const motionRef = useMotionRef(ref);
    return (_jsx(RP.Portal, { children: _jsx(RP.Content, { ref: motionRef, sideOffset: 6, ...props, className: cn("gdg-popup", "gdg-popover", className) }) }));
}
...
export function DropdownMenuContent({ ref, className, ...props }) {
    const motionRef = useMotionRef(ref);
    return (_jsx(RM.Portal, { children: _jsx(RM.Content, { ref: motionRef, sideOffset: 4, ...props, className: cn("gdg-popup", "gdg-menu", className) }) }));
}
...
export function TooltipContent({ ref, className, ...props }) {
    const motionRef = useMotionRef(ref);
    return (_jsx(RT.Portal, { children: _jsx(RT.Content, { ref: motionRef, sideOffset: 6, ...props, className: cn("gdg-popup", "gdg-tooltip", className) }) }));
}
